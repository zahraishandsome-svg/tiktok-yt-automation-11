        ...
        while True:
            ...
            if upload_mode == "dual":
                ...
                if slot == 2:
                    ...
                    _run_short_only(channel, video, slot, run_id, dry_run, result)
            elif upload_mode in ("popular_split", "popular_only", "sequence"):
...
def _pick_next_video(channel: Dict[str, Any], slot: int,
    ...
    # sequence: walk an explicit, ordered list of TikTok IDs (a series, in order).
    if upload_mode == "sequence":
        return _pick_from_sequence(channel, exclude_ids=exclude_ids)
...
def _days_since_last_upload(channel_id: str) -> Optional[int]:
    """Whole days since this channel's most recent successful upload, or None
    if it has never uploaded."""
    last = db.get_last_upload_date(channel_id)
    if not last:
        return None
    return (date.today() - last).days


def _pick_from_sequence(channel: Dict[str, Any],
                        exclude_ids: Optional[set] = None) -> Optional[Dict[str, Any]]:
    """
    Walk `video_sequence` (an ordered list of TikTok video IDs) and return the
    first one not yet posted. Used to release a multi-part series in order.

    `min_days_between_uploads` gates the cadence: the cron can fire daily and
    this returns None until the gap has elapsed, so the series paces itself
    without needing an every-N-days cron expression.
    """
    channel_id = channel["id"]
    exclude_ids = exclude_ids or set()

    sequence = [str(v) for v in (channel.get("video_sequence") or [])]
    if not sequence:
        logger.warning("[%s] upload_mode=sequence but video_sequence is empty", channel_id)
        return None

    gap = int(channel.get("min_days_between_uploads") or 0)
    if gap > 0:
        since = _days_since_last_upload(channel_id)
        if since is not None and since < gap:
            logger.info("[%s] Sequence paced: last upload was %d day(s) ago, "
                        "waiting for a %d-day gap", channel_id, since, gap)
            return None

    already_posted = db.get_posted_video_ids(channel_id, upload_mode="sequence")
    next_id = next((v for v in sequence
                    if v not in already_posted and v not in exclude_ids), None)
    if next_id is None:
        logger.info("[%s] Sequence complete — all %d video(s) posted",
                    channel_id, len(sequence))
        return None
    ...
    video = next((v for v in (all_videos or []) if str(v["id"]) == next_id), None)
    if video is None:
        logger.error("[%s] Sequence video %s is no longer on @%s — skipping it",
                     channel_id, next_id, tiktok_user)
        return None

    position = sequence.index(next_id) + 1
    db.record_video_seen(channel_id, video, format_type="longform")
    logger.info("[%s] Sequence pick %d/%d: %s | '%s'",
                channel_id, position, len(sequence), next_id,
...
def _resolve_title(channel: Dict[str, Any], video: Dict[str, Any]) -> str:
    ...
    """
    meta = (channel.get("video_meta") or {}).get(str(video["id"])) or {}
    custom = (meta.get("title") or "").strip()
    if custom:
        return custom
...
def _resolve_description(channel: Dict[str, Any], video: Dict[str, Any]) -> str:
    """Per-video description from `video_meta[<id>].description` when present,
    else the TikTok caption."""
    meta = (channel.get("video_meta") or {}).get(str(video["id"])) or {}
    return meta.get("description") or video.get("description") or ""
...
def _upload_video(channel: Dict[str, Any], video: Dict[str, Any],
    ...
    return upload_video(
        ...
        description=_resolve_description(channel, video),
