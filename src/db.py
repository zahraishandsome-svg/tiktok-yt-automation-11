...
def get_last_upload_date(channel_id: str):
    """
    Date of this channel's most recent successful upload, or None if it has
    never uploaded. Used by `sequence` mode to pace a series N days apart.
    """
    from datetime import date as _date
    conn = get_connection()
    row = conn.execute("""
        SELECT MAX(date(posted_at)) FROM posted_videos
        WHERE channel_id = ? AND status = 'uploaded' AND posted_at IS NOT NULL
    """, (channel_id,)).fetchone()
